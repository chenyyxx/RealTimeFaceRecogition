...
class face_annoy:
    ...
    def __init__(self):
        self.f = int(common.get_conf('annoy', 'face_vector'))
        self.annoy_index_path = os.path.abspath(common.get_conf('annoy', 'index_path'))
        self.lmdb_file = os.path.abspath(common.get_conf('lmdb', 'lmdb_path'))
        self.num_trees = int(common.get_conf('annoy', 'num_trees'))
    ...
    def create_index_from_lmdb(self):
        ...
        if os.path.isdir(lmdb_file):
            ...
            for key, value in wfp.cursor():
                print(key)
                key = int(key)
                value = common.str_to_embed(value)
                print(type(value))
    ...
    def query_vector(self, face_vector):
        n = int(common.get_conf('annoy', 'num_nn_nearst'))
        return self.annoy.get_nns_by_vector(face_vector, n, include_distances=True)

    def query_key(self, key):
        n=1
        return self.annoy.get_nns_by_item(key, n, include_distances=False)

if __name__ == '__main__':
    annoy = face_annoy()
    # annoy.create_index_from_lmdb()
    annoy.annoy.load(annoy.annoy_index_path)
    print(annoy.annoy.get_nns_by_item(0, 5))
    # annoy.query_vector()